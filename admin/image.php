...
class uploader{
	...
	private function copyFile(){
		...
		if(copy($this->source['tmp_name'],$this->destDir . $this->pre . $this->source['name'])){
			// Done.
			$this->info .= '<div>file was uploaded successfully.</div>';
			return $this->pre . $this->source['name'];
		} else {
	...
	private function fileExists(){
		$this->preDir();
		if(file_exists($this->destDir. $this->pre .$this->source['name'])){
	 ...
	 * @return String fileName or False on error
	 */
	public function resize($file='',$width='',$height='', $crop = false){
		if($file!=""){ $this->source = $file; }
		if($width != '') $this->newWidth = $width;
		if($height != '') $this->newHeight = $height;
		if($crop){
			return $this->_resizeAndcrop();
		}		
	 ...
	 */
	private function _resizeAndcrop (){
		...
		if(strtolower(trim($ext)) == "jpg" || strtolower(trim($ext)) == "jpeg")
		{
		...
		$width_new = $height * $this->newWidth / $this->newHeight;
		$height_new = $width * $this->newHeight / $this->newWidth;
		//if the new width is greater than the actual width of the image, then the height is too large and the rest cut off, or vice versa
		if($width_new > $width){
			//cut point by height
			$h_point = (($height - $height_new) / 2);
			//copy image
			imagecopyresampled($normal, $src, 0, 0, 0, $h_point, $this->newWidth, $this->newHeight, $width, $height_new);
		}else{
			//cut point by width
			$w_point = (($width - $width_new) / 2);
			imagecopyresampled($normal, $src, 0, 0, $w_point, 0, $this->newWidth, $this->newHeight, $width_new, $height);
		}
		$dir = $this->resizeDir;
